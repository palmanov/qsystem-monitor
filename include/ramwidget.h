...
class RAMStatWidget : public QWidget {
  Q_OBJECT
 public:
  explicit RAMStatWidget(QWidget* parent = nullptr)
      : QWidget(parent) {
 ...
 public slots:
  void setRAMUsage(uint free, uint total, uint cached) {
    free_label_->setText(QString("%1 G").arg(QString::number(free >> 20, 'f', 1)));
    used_label_->setText(QString("%1 G").arg(QString::number((total - free - cached) >> 20, 'f', 1)));
    cached_label_->setText(QString("%1 G").arg(QString::number(cached >> 20, 'f', 1)));
  }
