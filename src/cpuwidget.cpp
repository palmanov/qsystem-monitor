...
constexpr QColor kSysColor = QColor(235, 85, 69);
constexpr QColor kUserColor = QColor(107, 212, 95);
constexpr QColor kIOWaitColor = QColor(248, 216, 74);
constexpr QColor kGuestColor = QColor(178, 96, 234);
constexpr QColor kIdleColor = QColor(70, 70, 70);
...
enum class CPUState { kSys = 0, kUser, kIOWait, kGuest };

class CPUStatsWidget : public QWidget {
 public:
  explicit CPUStatsWidget(QWidget* parent = nullptr) : QWidget(parent) {
    auto cpu_info = parseProcCpuInfo({"siblings"});
    const auto siblings = cpu_info["siblings"].toInt();
    this->setMinimumSize(kItemsCount * kItemW +
                             (kItemsCount - 1) * kItemsSpacing,
                         (siblings - 1) * kItemsSpacing + kItemH * siblings);
  }
 ...
 public:
  void setCPUStats(const QVector<CPUStats>& stats) {
    ...
    for (size_t i = 1; i < stats.size(); i++) {
      ...
      const auto total = stat.user + stat.nice + stat.system + stat.idle +
                         stat.iowait + stat.irq + stat.softirq + stat.steal +
                         stat.guest + stat.guest_nice;

      stat_perc.sys =
          100.f * (stat.system - prev_stat.system) / (total - prev_total_time);
      stat_perc.user =
          100.f * (stat.user - prev_stat.user) / (total - prev_total_time);
      stat_perc.iowait =
          100.f * (stat.iowait - prev_stat.iowait) / (total - prev_total_time);
      stat_perc.steal =
          100.f * (stat.steal - prev_stat.steal) / (total - prev_total_time);
      stat_perc.guest =
          100.f * (stat.guest - prev_stat.guest) / (total - prev_total_time);
      stat_perc.idle =
          100.f * (stat.idle - prev_stat.idle) / (total - prev_total_time);
 ...
 protected:
  void paintEvent(QPaintEvent* e) override {
    ...
    for (size_t i = 1; i < cpu_stats_percent_.size(); i++) {
      ...
      const int system_items_count =
          std::round(cpu_stats_percent_[i].sys / item_percent);
      const int user_items_count =
          std::round(cpu_stats_percent_[i].user / item_percent);
      const int iowait_items_count =
          std::round(cpu_stats_percent_[i].iowait / item_percent);
      const int guest_items_count =
          std::round(cpu_stats_percent_[i].guest / item_percent);
      const int idle_items_count =
          kItemsCount - (system_items_count + user_items_count +
                         iowait_items_count + guest_items_count);

      int x_offset = 0;
      for (int k = 0; k < system_items_count; k++) {
        painter.setBrush(kSysColor);
        painter.drawRoundedRect(x_offset,
                                (i - 1) * kItemsSpacing + kItemH * (i - 1),
                                kItemW, kItemH, 3, 3);
...
class CPUStateLabel : public QLabel {
 public:
  explicit CPUStateLabel(CPUState state, const QString &str,
                         QWidget *parent = nullptr)
      : QLabel(str, parent), state_(state) {}
 ...
 protected:
  void paintEvent(QPaintEvent *e) override {
    ...
    switch (state_) {
      case CPUState::kSys:
        brush_color = kSysColor;
        break;
      case CPUState::kIOWait:
        brush_color = kIOWaitColor;
        break;
      case CPUState::kGuest:
        brush_color = kGuestColor;
        break;
      case CPUState::kUser:
        brush_color = kUserColor;
        break;
      default:
        break;
    }
    ...
    const QRect rectangle =
        QRect(kItemW + 4, 0, this->width() - kItemW + 4, this->height());
...
CPUWidget::CPUWidget(QWidget* parent) : QWidget(parent) {
  ...
  top_layout->addWidget(cpu_util_label_);
  ...
  l->addLayout(bottom_layout);
  this->setLayout(l);
}

void CPUWidget::setCPUStats(const QVector<CPUStats>& stats) {
  ...
  stat_perc.sys =
      100.f * (stat.system - prev_cpu_stats.system) / (total - prev_total_time);
  stat_perc.user =
      100.f * (stat.user - prev_cpu_stats.user) / (total - prev_total_time);
  stat_perc.iowait =
      100.f * (stat.iowait - prev_cpu_stats.iowait) / (total - prev_total_time);
  stat_perc.steal =
      100.f * (stat.steal - prev_cpu_stats.steal) / (total - prev_total_time);
  stat_perc.idle =
      100.f * (stat.idle - prev_cpu_stats.idle) / (total - prev_total_time);

  cpu_util_label_->setText(
      QString("%1%").arg(static_cast<int>(100.0 - stat_perc.idle)));
  cpu_sys_label_->setText(QString("%1%").arg(static_cast<int>(stat_perc.sys)));
  cpu_user_label_->setText(
      QString("%1%").arg(static_cast<int>(stat_perc.user)));
  cpu_iowait_label_->setText(
      QString("%1%").arg(static_cast<int>(stat_perc.iowait)));
  cpu_guest_label_->setText(
      QString("%1%").arg(static_cast<int>(stat_perc.steal)));

  prev_total_time = total;
  prev_cpu_stats = stats[0];
}

void CPUWidget::setUptime(uint uptime) {
...
void CPUWidget::setLoadAvg(const QVector<double> &load_avg) {
  avg_load_label_->setText(
      QString("%1 %2 %3").arg(load_avg[0]).arg(load_avg[1]).arg(load_avg[2]));
}
